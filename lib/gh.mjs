...
export async function prStatusChecks(n) {
  ...
  try {
    ...
    // GitHub API 返回小写（queued/in_progress/failure/success），统一转大写方便 bot 比较
    return (data.check_runs ?? []).map((c) => ({ name: c.name, state: (c.status ?? "").toUpperCase(), conclusion: (c.conclusion ?? "").toUpperCase() }));
  } catch {
    return [];
  }
}

/** 失败 check 的详情（check-run output 摘要，供迭代修复时给模型看） */
export async function prFailedCheckDetails(n) {
  try {
    const pr = await gh(["pr", "view", String(n), "--json", "headRefOid"], { data: true });
    const sha = pr.headRefOid;
    const { stdout } = await exec("gh", ["api", `repos/${REPO}/commits/${sha}/check-runs`], { encoding: "utf8" });
    ...
    return (data.check_runs ?? [])
      ...
      .map((c) => `### ${c.name}\n${(c.output?.text || c.output?.summary || "(无输出)").slice(0, 2000)}`)
      .join("\n\n")
      .slice(0, 6000);
  } catch {
    return "";
